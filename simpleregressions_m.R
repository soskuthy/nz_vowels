...
#### PRICE
price = readRDS("final_data/price_averaged.rds")
# this file contains the wrong data
pricef = readRDS("final_data/price_full.rds")
...
plot_sample <- function (dat, n) {
  spes <- filter(dat, speaker %in% sample(unique(dat$speaker), n, replace=F))
  ggplot(spes, aes(x=measurement_no, y=f1_avg, col=following_voiceless)) +
    facet_wrap(~speaker) +
    geom_line() +
    geom_line(aes(y=f2_avg))
}

# find the max f1 and the max f2.  then cross reference with original file to # get measurement point and demographics.

# M: let's try finding minima / maxima along smooths fitted to the raw data...

find_inflection <- function (xs, ys, k, groups, convex=T, n=1000, plot=F) {
  d <- data.frame(xs, ys, groups)
  d <- d[order(groups, xs),]
  d$start <- d$xs == min(d$xs)
  d$groups <- as.factor(d$groups)
  
  # fit gamm with CAR1 continuous autoregressive error model
  modAR <- gamm(ys ~ s(xs, k=k, bs="cr"),
              data=d, correlation=corAR1(form=~ xs | groups))
  #modAR <- gam(ys ~ s(xs, k=k, bs="cr", sp=5000),
  #              data=d)
  #modAR <- list(gam=modAR)
...
spe <- filter(pricef, speaker=="mop94-9a", following_voiceless==T)
xs <- spe$measurement_no; ys <- spe$f1; k=10; groups=spe$id; n=1000
find_inflection(xs, ys, k, groups, convex=F, plot=T)

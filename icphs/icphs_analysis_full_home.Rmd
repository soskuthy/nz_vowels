...
find_inflection_traj <- function (xs, ys, id, k, convex=T, n=1000, plot=F, .pb=NULL) {
  if ((!is.null(.pb)) && inherits(.pb, "Progress") && (.pb$i < .pb$n)) 
    .pb$tick()$print()
  d <- data.frame(xs, ys)
  d <- d[order(xs),]
  if (k > length(unique(xs))) {
    if (length(unique(xs)) < 9) {
      #warning(paste0('trajectory has too many missing points (more > 2), returning NA'))
      return(NA)
    }
    k <- length(unique(xs))
    warning(paste0('k reduced to ', k))
  ...
  # fit gamm with AR1 autoregressive error model
  modAR <- tryCatch({
    gam(ys ~ s(xs, k=k, bs="cr"),
              data=d)
    },
    error=function (x) NULL
  )
  if (is.null(modAR)) {
    #warning("model could not be fit to trajectory")
    return(NA)
  }
  ...
  # second derivative
  second_deriv <- Xpp %*% coef(modAR)
  smooth_second_deriv <- predict(gam(second_deriv~s(new_xs)))
...
pb = progress_estimated(length(unique(price_vd$id)) * 2)
price_vd_infl <- price_vd %>%
  group_by(id) %>%
  summarise(dur=dur[1],
            f1_infl=find_inflection_traj(measurement_no, 
                                      bark(f1),
                                      id=id[1],
                                      k=10,
                                      plot=F, convex=F),
            f2_infl=find_inflection_traj(measurement_no, 
                                      bark(f2),
                                      id=id[1],
                                      k=10,
                                      plot=F, convex=T),
            yob=yob[1], sex=sex[1], previous=previous[1], following=following[1],
            wordform=wordform[1], speaker=speaker[1]) %>%
  ungroup()

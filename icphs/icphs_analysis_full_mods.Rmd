---
title: "ICPhS analysis of NZE diphthongs - no averaging"
...
## GAM modelling of contours

Eschewing one big model strategy, and fitting separate models to price & mouth.

```{r}
price_vd$dur_log <- log(price_vd$dur)
price_vd$previous_f <- factor(price_vd$previous)
price_vd$following_f <- factor(price_vd$following)
price_vd$speaker_f <- factor(price_vd$speaker)

price_vd$sex <- factor(price_vd$sex)
...
price_vd_sub <- price_vd[price_vd$speaker %in% sample(unique(price_vd$speaker), 50, replace=F),]

system.time({
  price_f1_mod_no_AR <- 
    bam(f1 ~ sex +
          s(dur_log, k=5, by=sex) +
          s(measurement_no, k=11, by=sex) + 
          s(yob, by=sex, k=10) +
          ti(measurement_no, yob, k=c(11,10), by=sex) +
          ti(measurement_no, dur_log, k=c(11,5), by=sex) +
          s(measurement_no, previous_f, bs="fs", m=1, k=10) +
          s(measurement_no, following_f, bs="fs", m=1, k=10) +
          s(measurement_no, speaker_f, bs="fs", m=1, k=10),
        data=price_vd_sub, discrete=T, nthreads=6)})

rho_est <- start_value_rho(price_f1_mod_no_AR)
...
system.time({
  price_f1_mod <- 
    bam(f1 ~ sex +
          ...
          s(measurement_no, following_f, bs="fs", m=1, k=10) +
          s(measurement_no, speaker_f, bs="fs", m=1, k=10),
        data=price_vd, discrete=T, nthreads=6, 
        rho=rho_est, AR.start=price_vd$AR_start)})
#summary(price_f1_mod)
saveRDS(price_f1_mod, "../models/price_f1_mod.rds")
## F2 models
...
rho_est <- start_value_rho(price_f2_mod_no_AR)
...
system.time({
  price_f2_mod <- 
    bam(f2 ~ sex +
        ...
        data=price_vd, discrete=T, nthreads=6, 
        rho=rho_est, AR.start=price_vd$AR_start)})
saveRDS(price_f2_mod, "../models/price_f2_mod.rds")
```


```{r}
mouth_vd$dur_log <- log(mouth_vd$dur)
mouth_vd$previous_f <- factor(mouth_vd$previous)
mouth_vd$following_f <- factor(mouth_vd$following)
mouth_vd$speaker_f <- factor(mouth_vd$speaker)

mouth_vd$sex <- factor(mouth_vd$sex)
...
mouth_vd_sub <- mouth_vd[mouth_vd$speaker %in% sample(unique(mouth_vd$speaker), 50, replace=F),]
...
system.time({
  mouth_f1_mod_no_AR <- 
    bam(f1 ~ sex +
          ...
          s(measurement_no, following_f, bs="fs", m=1, k=10) +
          s(measurement_no, speaker_f, bs="fs", m=1, k=10),
        data=mouth_vd_sub, discrete=T, nthreads=6)})

rho_est <- start_value_rho(mouth_f1_mod_no_AR)
...
system.time({
  mouth_f1_mod <- 
    bam(f1 ~ sex +
          ...
          s(measurement_no, following_f, bs="fs", m=1, k=10) +
          s(measurement_no, speaker_f, bs="fs", m=1, k=10),
        data=mouth_vd, discrete=T, nthreads=6, 
        rho=rho_est, AR.start=mouth_vd$AR_start)})
#summary(mouth_f1_mod)
saveRDS(mouth_f1_mod, "../models/mouth_f1_mod.rds")
## F2 models
...
rho_est <- start_value_rho(mouth_f2_mod_no_AR)
...
system.time({
  mouth_f2_mod <- 
    bam(f2 ~ sex +
        ...
        data=mouth_vd, discrete=T, nthreads=6, 
        rho=rho_est, AR.start=mouth_vd$AR_start)})
saveRDS(mouth_f2_mod, "../models/mouth_f2_mod.rds")
```
...
newdat <- expand.grid(measurement_no=seq(1,11,length.out=100),
                      yob=seq(min(price_vd$yob),
                              max(price_vd$yob),
                              1),
                      sex=unique(price_vd$sex),
                      dur_log=median(price_vd$dur_log),
                      previous_f=price_vd$previous_f[1],
                      following_f=price_vd$following_f[1],
                      speaker_f=price_vd$speaker_f[1],
                      f1=0, f2=0)
...
preds_f1_fix <- preds_f1[,-grep("s[(]measurement_no,.*_f[)]", colnames(preds_f1))]
preds_f2_fix <- preds_f2[,-grep("s[(]measurement_no,.*_f[)]", colnames(preds_f2))]
coefs_f1_fix <- coef(price_f1_mod)[-grep("s[(]measurement_no,.*_f[)]", names(coef(price_f1_mod)))]
coefs_f2_fix <- coef(price_f2_mod)[-grep("s[(]measurement_no,.*_f[)]", names(coef(price_f2_mod)))]



newdat$f1 <- preds_f1_fix %*% coefs_f1_fix
newdat$f2 <- preds_f2_fix %*% coefs_f2_fix
...
newdat1$hz <- newdat1$f1
newdat2$hz <- newdat2$f2
newdat1$formant <- "f1"
newdat2$formant <- "f2"
...
for (v in unique(newdat$vowel)) {
  for (s in unique(newdat$sex)) {
    ...
    dd <- unique(dplyr::select(newdat, vowel, sex))

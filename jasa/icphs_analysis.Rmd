...
price_avgs <- readRDS("final_data/price_averaged_jasa.rds")
mouth <- readRDS("final_data/mouth_full_jasa.rds")
mouth_avgs <- readRDS("final_data/mouth_averaged_jasa.rds")

# for compatibility, convert measurement number to 1-11
price$measurement_no <- 1 + price$measurement_no*10
price_avgs$measurement_no <- 1 + price_avgs$measurement_no*10
mouth$measurement_no <- 1 + mouth$measurement_no*10
mouth_avgs$measurement_no <- 1 + mouth_avgs$measurement_no*10
...
price_vd_infl <- price_vd %>%
  ...
  summarise(dur_avg=mean(dur),
            f1_infl=find_inflection(measurement_no, 
                                      bark(f1),
                                      k=10,
                                      random_terms=as.data.frame(cbind(previous,following)),
                                      groups=id,

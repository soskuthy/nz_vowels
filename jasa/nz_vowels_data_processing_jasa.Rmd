...
price <- price_wide %>%
  ...
  mutate(id = paste0(number, "_", speaker),
...
price_filtered1 <- readRDS("../final_data/price_full_jasa.rds")

anti_join(price_filtered %>% select(-id, -id_str, -number), price_filtered1 %>% select(-id))
